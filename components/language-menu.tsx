...
const languages = [['zh-Hans', '简体中文'], ['zh-Hant', '繁體中文'], ['en', 'English'], ['fr', 'Français'], ['es', 'Español'], ['ja', '日本語'], ['hi', 'हिन्दी'], ['he', 'עברית']] as const;
export function LanguageMenu({locale, path = ''}: {locale: BrandLocale; path?: string}) {
  ...
  const label = {'zh-Hant':'選擇語言', 'zh-Hans':'选择语言', en:'Choose language', fr:'Choisir la langue', es:'Elegir idioma', hi:'भाषा चुनें', ja:'言語を選択', he:'בחירת שפה'}[locale];
  const cleanPath = path.replace(/^\/+|\/+$/g, '');
  return <details ref={root} className="language-menu"><summary aria-label={label}><span aria-hidden="true">🌐</span><span className="language-current">{languages.find(([code]) => code === locale)?.[1]}</span><span aria-hidden="true">⌄</span></summary><nav className="language-options" aria-label={label}>{languages.map(([code, name]) => <a key={code} href={`${code === 'zh-Hans' ? '' : '/' + code}/${cleanPath}`.replace(/\/$/, '') || '/'} hrefLang={code} lang={code} dir={code === 'he' ? 'rtl' : 'ltr'} aria-current={code === locale ? 'page' : undefined}>{name}{code === locale ? ' ✓' : ''}</a>)}</nav></details>;
}
