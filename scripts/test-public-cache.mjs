...
const make=(path='/',headers={},method='GET',host='isun1.com')=>new Request(`https://${host}${path}`,{method,headers:{accept:'text/html',...headers}});
const waits=[]; const ctx={waitUntil(p){waits.push(p)}};
assert(canCache(make()));assert(canCache(make('/he')));assert(canCache(make('/zh-Hant')));assert(canCache(make('/fr/global')));assert(canCache(make('/hi/robin')));assert(canCache(make('/',{cookie:'__cf_bm=public-bot-check'})));assert.equal(canCache(make('/',{cookie:'__cf_bm=public; session=private'})),false);assert(canCache(make('/ja/robin')));
for(const req of [make('/contact'),make('/verify?id=ISUN-1'),make('/api/contact'),make('/search?q=Robin'),make('/?q=x'),make('/',{cookie:'a=b'}),make('/',{authorization:'Bearer test'}),make('/',{rsc:'1'}),make('/',{'next-router-state-tree':'x'}),make('/',{'next-url':'/en'}),make('/',{'x-vinext-interception-id':'x'}),make('/',{accept:'text/x-component'}),make('/missing'),make('/',{},'POST'),make('/',{},'GET','www.isun1.com'),make('/',{},'GET','isun1-suntv.robin10.chatgpt.site')]) assert.equal(canCache(req),false,req.url);
