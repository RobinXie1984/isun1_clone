...
const allowedOrigins = new Set([
  'https://isun1.com',
  'https://www.isun1.com',
  'https://isun1-suntv.robin10.chatgpt.site',
...
export async function POST(request: Request) {
  ...
  if (
    ...
    return reply(
      'Email delivery is not configured. Contact admin@tideisun.com.',
      503,
    );
  try {
    if (config.GOOGLE_WORKSPACE_RELAY_URL && config.GOOGLE_WORKSPACE_RELAY_SECRET) {
      ...
      if (result.reason === 'rate-limit') return reply('Please try later or email admin@tideisun.com.', 429);
    ...
    if (!response.ok)
      return reply('Delivery failed. Please email admin@tideisun.com.', 502);
