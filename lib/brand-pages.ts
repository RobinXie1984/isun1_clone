...
export const pageDescriptions: Record<string, [string, string]> = {
  ...
  "contact": [
    "聯絡陽光衛視洽談華商出海、採訪及品牌合作。香港總部與集團全球辦公室；商務信箱 admin@tideisun.com。",
    "Contact iSunTV for international collaboration, interviews and brand partnerships. Hong Kong headquarters and group offices; admin@tideisun.com."
  ],
  "privacy": [
...
export function brandMetadata(
  ...
  return {
    ...
    alternates: {
      canonical: `https://isun1.com${brandPath(path, locale)}`,
      languages: Object.fromEntries([...brandLocales.map(l => [l, `https://isun1.com${brandPath(path, l)}`]), ['x-default', `https://isun1.com${brandPath(path, 'zh-Hans')}`]]),
    },
    openGraph: {
      title,
      description,
      ...
      url: `https://isun1.com${brandPath(path, locale)}`,
    },
