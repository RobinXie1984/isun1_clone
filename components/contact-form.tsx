    ...
    <form
      ...
      onSubmit={async (e) => {
        ...
        try {
          ...
          setState('error');
          setMessage(
            t('目前無法確認寄送結果。請直接寄信至 admin@tideisun.com。', 'Delivery could not be confirmed. Please email admin@tideisun.com directly.'),
          );
        }
      }}
    >
      {enabled && !ready && <p className="status-message">{t('正在載入表單，亦可直接寄信至 admin@tideisun.com。', 'Loading the form. You can also email admin@tideisun.com.')}</p>}
      {!enabled && (
        <p className="status-message">
          {t('請直接寄信至 admin@tideisun.com 洽談合作。網上提交尚未開放。', 'For enquiries, please email admin@tideisun.com directly. Online submission is not yet available.')}
        </p>
      )}
      <fieldset
      ...
      >
        ...
        <label className="checkbox-label">
          ...
          <span>
            ...
            <a href={`${locale === 'zh-Hans' ? '' : '/' + locale}/privacy`}>
