...
export function checkedLocale(value: string | undefined): Locale {
  if (!locales.includes(value as Locale) || value === 'zh-Hans') notFound();
...
export function languageAlternates(path: string, locale?: Locale) {
  return {
    ...
    languages: Object.fromEntries(
      [...locales.filter(l => !path.startsWith('videos/') || Boolean(drafts[path.slice(7)]?.fields[l])).map((l) => [l, new URL(sitePath(l, path), publicOrigin).href]), ['x-default', new URL(sitePath('zh-Hans', path), publicOrigin).href]],
    ),
  };
}
